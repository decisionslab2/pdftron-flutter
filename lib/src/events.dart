...
const _toolChangedChannel = const EventChannel('tool_changed_event');
...
/// A listener used as the argument for [startToolChangedListener].
///
/// Gets the [toolMode] string identifier of the newly selected tool.
typedef void ToolChangedListener(dynamic toolMode);
...
/// Listens for when the active annotation tool is changed.
///
/// ```dart
/// var toolChangedCancel = startToolChangedListener((toolMode) {
///   print('flutter tool changed to: $toolMode');
/// });
/// ```
///
/// Returns a function that can cancel the listener.
CancelListener startToolChangedListener(ToolChangedListener listener) {
  var subscription = _toolChangedChannel
      .receiveBroadcastStream(eventSinkId.toolChangedId.index)
      .listen(listener, cancelOnError: true);
...
/// Listens for Decisions-specific share annotation events.
///
/// Returns a function that can cancel the listener.
